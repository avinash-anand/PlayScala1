  ...
  "ReactiveMongoService" must {
    ...
    "create" must {
      "for valid JSON, return status CREATED(201)" in {
        ...
        when(mockCollection.insert(Matchers.eq(TestData("Avinash")), Matchers.any())(Matchers.any(), Matchers.any())).thenReturn(Future.successful(LastError(true, None, None, None, None, 0, false)))
